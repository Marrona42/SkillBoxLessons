...
namespace GameOfLife
{
    ...
    public class LifeSimulation
    {
            ...
            for (int i = 0; i < _heigth; i++)
            {
                for (int j = 0; j < _width; j++)
                {
                    ...
                    if (cells[i, j])
                    {
                        ...
                        if (age > 2) //если возраст клетки 2 или больше, она умирает
                        {
                            cells[i, j] = false; 
                        }
                    }
        ...
        /// Определяем возраст клетки. 
        /// Если её итак нет, обнуляем значение возвраста
       
        
         private int HowOld(int x, int y)
        {
            int age = 0;
